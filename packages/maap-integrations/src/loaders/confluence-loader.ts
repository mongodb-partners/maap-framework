...
export class ConfluenceLoader extends BaseLoader<{ type: "ConfluenceLoader" }> {
  ...
  constructor({
    ...
    super(
      `ConfluenceLoader_${md5(spaceNames.join(","))}`,
      chunkSize ?? 2000,
      chunkOverlap ?? 200,
    );
    ...
    assert(
      typeof confluenceBaseUrl === "string" ||
        typeof process.env.CONFLUENCE_BASE_UR === "string",
  ...
  override async *getUnfilteredChunks() {
    for (const spaceKey of this.spaceNames) {
      try {
        const spaceContent = await this.confluence.space.getContentForSpace({
          spaceKey,
        });
        this.debug(
        ...
        for await (const result of this.getContentChunks(
          spaceContent["page"].results,
        )) {
          yield result;
        }
      } catch (e) {
