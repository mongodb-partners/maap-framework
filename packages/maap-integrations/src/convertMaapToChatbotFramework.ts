...
export async function convertBaseModelToChatLlm(
  baseModel: BaseModel,
): Promise<ChatLlm> {
...
export function convertBaseEmbeddingsToEmbedder(
...
export function convertBaseLoaderToDataSource(
...
export function convertBaseRerankerToReranker(
  ...
  return async ({ query, results }) => {
    ...
    for (const result of rerankedResults) {
      ...
      if (foundRes === -1) {
        throw new Error(
          "Could not find the original result in the reranked results",
        );
      }
