...
export class RAGApplication {
  ...
  constructor(llmBuilder: RAGApplicationBuilder) {
    ...
    RAGEmbedding.init(
      llmBuilder.getEmbeddingModel() ?? new NomicEmbeddingsv1_5(),
    );
  ...
  public async init() {
    ...
    await this.vectorDb.init({
      dimensions: RAGEmbedding.getEmbedding().getDimensions(),
    });
  ...
  private async batchLoadEmbeddings(
    ...
    this.debug(
      `Batch embeddings (size ${formattedChunks.length}) obtained for loader`,
      loaderUniqueId,
    );
  ...
  private async incrementalLoader(
    uniqueId: string,
    incrementalGenerator: AsyncGenerator<LoaderChunk, void, void>,
  ) {
    this.debug(`incrementalChunkAvailable for loader`, uniqueId);
    const { newInserts } = await this.batchLoadChunks(
      uniqueId,
  ...
  public async addLoader(loader: BaseLoader): Promise<AddLoaderReturn> {
    ...
    if (this.cache && (await this.cache.hasLoader(uniqueId))) {
      ...
      this.debug(
        `Loader previously run. Deleting previous ${previousChunkCount} keys`,
        uniqueId,
      );
    ...
    const { newInserts, formattedChunks } = await this.batchLoadChunks(
      uniqueId,
      chunks,
    );
    if (this.cache)
    ...
    this.debug(
      `Add loader completed with ${newInserts} new entries for`,
      uniqueId,
    );
    ...
    this.loaders = this.loaders.filter(
      (x) => x.getUniqueId() != loader.getUniqueId(),
    );
  ...
  public async deleteLoader(
    uniqueLoaderId: string,
    areYouSure: boolean = false,
  ) {
    if (!areYouSure) {
      console.warn(
        "Delete embeddings from loader called without confirmation. No action taken.",
      );
      return false;
    }
    ...
    this.loaders = this.loaders.filter(
      (x) => x.getUniqueId() != uniqueLoaderId,
    );
    return deleteResult;
  }
  ...
  public async deleteAllEmbeddings(areYouSure: boolean = false) {
    if (!areYouSure) {
      console.warn(
        "Reset embeddings called without confirmation. No action taken.",
      );
      return false;
    }
  ...
  public async getEmbeddings(cleanQuery: string) {
    const queryEmbedded =
      await RAGEmbedding.getEmbedding().embedQuery(cleanQuery);
    let unfilteredResultSet = await this.vectorDb.similaritySearch(
      queryEmbedded,
      this.searchResultCount + 10,
    );
    if (this.reranker) {
      const rerankedResultSet = await this.reranker.reRankDocuments(
        cleanQuery,
        unfilteredResultSet,
      ); // Pass cleanQuery as the first argument
  ...
  public async getContext(query: string) {
    ...
    return [
      ...new Map(rawContext.map((item) => [item.pageContent, item])).values(),
    ];
  }
